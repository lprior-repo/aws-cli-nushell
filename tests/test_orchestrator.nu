...
#[test]
def suite-execution-strategy-validation [] {
    let context = $in
    let temp = $context.temp
    
    let suite = "assert equal 1 1" | create-single-test-suite $temp "strategy_test"
    let suites = [{ name: $suite.name, path: $suite.path, tests: $suite.tests }]
    
    # Test with different strategy settings
    $suites | run-suites (noop-event-processor) { threads: 1 }
    
    let results = store query | where suite in ($suites | get name)
    assert equal ($results | length) 1
}

#[test]
def event-processor-integration [] {
    let context = $in
    let temp = $context.temp
    
    let suite = "assert equal 1 1" | create-single-test-suite $temp "event_test"
    let suites = [{ name: $suite.name, path: $suite.path, tests: $suite.tests }]
    
    # Create a mock event processor that captures events
    let event_processor = {
        run-start: { || print "run-start-event" }
        run-complete: { || print "run-complete-event" }
        test-start: { |row| print $"test-start-($row.test)" }
        test-complete: { |row| print $"test-complete-($row.test)" }
    }
    
    $suites | run-suites $event_processor { threads: 1 }
...
#[test]
def resource-management-during-execution [] {
    let context = $in
    let temp = $context.temp
    
    # Create multiple suites to test resource management
    mut suite1 = create-suite $temp "resource1"
    let suite1 = "assert equal 1 1" | append-test $temp $suite1 "test1"
    
    mut suite2 = create-suite $temp "resource2"  
    let suite2 = "assert equal 1 1" | append-test $temp $suite2 "test2"
    
    let suites = [$suite1, $suite2]
    $suites | run-suites (noop-event-processor) { threads: 1 }
